<template>
  <div class="popup-wrapper">
    <div v-if="ready">
      <vue-qrcode class="qr" v-bind:value="url" v-bind:color="{dark: '#fff', light: '#0000'}" v-bind:width="250"
        v-bind:scale="1" v-bind:margin="0" />
      <div class="code">
        <h2>Your short link: <span @click="visitUrl" class="link">qrgen.cc/{{code}}</span></h2>
      </div>
      <div class="options">
        <p class="copy link" id="copy" @click="copy">Copy URL</p>
        <p class="or">or</p>
        <p @click="newShort" class="copy link">Create another one</p>
      </div>
    </div>
    <p v-if="!ready && !error">Loading...</p>
